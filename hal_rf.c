...
const HAL_RF_BURST_CONFIG myRfConfigCC1101 = {
  ...
  0x36,  // FOCCFG              Frequency Offset Compensation Configuration - debug experiment
...
const HAL_RF_BURST_CONFIG myRfConfigCC1101 = {
  ...
  0x07,  // MCSM2               Main Radio Control State Machine Configuration
  0x30,  // MCSM1               Main Radio Control State Machine Configuration - CCA - RSSI below threshold
  0x08,  // MCSM0               Main Radio Control State Machine Configuration
  0x16,  // FOCCFG              Frequency Offset Compensation Configuration
  //0x36,  // FOCCFG              Frequency Offset Compensation Configuration - debug experiment
  0x6C,  // BSCFG               Bit Synchronization Configuration
...
void halRfResetChip(void)
{
    ...
    GPIO_setOutputLowOnPin(GPIO_PORT_P1, GPIO_PIN7);
    while(GPIO_getInputPinValue(GPIO_PORT_P2, GPIO_PIN5));  // wait until MISO pin is low
...
void  halRfBurstConfig(void)
{
    ...
    halSpiWrite(CC1101_IOCFG2  | CC1101_WRITE_BURST, myRfConfigCC1101, 47);

    uint8_t rfPaTable = 0x8E;  // 0dBm
    halRfWriteReg(CC1101_PATABLE,rfPaTable);
}
...
void halRfWakeUpChip(void)
{
    ...
    GPIO_setOutputLowOnPin(GPIO_PORT_P1, GPIO_PIN7);
    myDelay();  // guess delay - need to adjust
